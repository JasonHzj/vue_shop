...
export default {
  ...
  methods: {
    drawChart () {
      ...
      var myChart = this.$echarts.init(document.getElementById(this.echarts), 'infographicN')
      // 绘制图表
    //   if(this.chartData.xList !== []){
    //   var straTime = this.$moment(this.chartData.xList[0]).format('MM.DD')
    //   var endTime = this.$moment(this.chartData.xList[this.chartData.xList.length-1]).format('MM.DD')
    //   }
      ...
      var option = {
        title: {
        //   text: straTime + "-" + endTime +this.chartData.titleName,
